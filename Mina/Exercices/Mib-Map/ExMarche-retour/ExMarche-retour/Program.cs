...
// Méthode pour exporter en CSV -- Fat par IA
void ExportToCsv(List<P2> data, string filePath)
{
    var csv = new StringBuilder();

    // En-tête CSV
    csv.AppendLine("ProducerAnon,Product,CA");

    // Données ligne par ligne
    foreach (var item in data)
    {
        // On échappe les éventuelles virgules dans les données avec des guillemets
        string producer = EscapeForCsv(item.ProducerAnon);
        string product = EscapeForCsv(item.Product);
        string ca = item.CA.ToString(System.Globalization.CultureInfo.InvariantCulture);

        csv.AppendLine($"{producer},{product},{ca}");
    }
...
// Petite fonction pour gérer les guillemets dans les valeurs CSV -- Fait par IA
...
//p => (name : p.Producer, product: p.ProductName, quantity: p.Quantity, price: p.PricePerUnit )


//var aliment = products.Select(p => i18n[key : p.ProductName]);
//var ca = products.Select(p => p.Quantity * p.PricePerUnit);

var newList = products
    .Select(p => new P2()
    {
        ProducerAnon = p.Producer.Substring(0, 3) + "..." + p.Producer[p.Producer.Length - 1],
        Product = i18n[key: p.ProductName],
...
ExportToCsv(newList, "export.csv");
