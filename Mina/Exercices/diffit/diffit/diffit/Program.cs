...
Func<string, string> cleanSpaces = text => {
    return text.Replace(" ", "");
};
Func<string, string> cleanTabs = text => {
    return text.Replace("\t", "");
};
Func<string, string> enforceCase = text => {
    return text.ToLower();
};
...
List<string> cleanA = linesA.ToList();
List<string> cleanB = linesB.ToList();


List<Func<string, string>> cleaners = new();
// TODO:  05 Appliquer le nettoyage selon la demande utilisateur
if(ignoreSpaces)
{
    cleaners.Add(cleanSpaces);
}
if(ignoreTabs)
{
    cleaners.Add(cleanTabs);
}
if(ignoreCase)
{
    cleaners.Add(enforceCase);
}

foreach(var cleaner in cleaners)
{
    cleanA = cleanA.Select(line => cleaner(line)).ToList();
    cleanB = cleanB.Select(line => cleaner(line)).ToList();
}
...
{
    var c =new LinesComparison() { Number = i, 
        ContentA = cleanA[i], 
        ContentB = cleanB[i] };
    comparisons.Add(c);
}
*/
comparisons = cleanA.Select((line,index) => new LinesComparison() { Number = index, ContentA = line, ContentB = cleanB[index] }).ToList();


// TODO: 07 Sélectionner les lignes qui ont des différences
var diffLines = new List<LinesComparison>();

diffLines = comparisons.Where(element => element.ContentA != element.ContentB).ToList();
...
Func<LinesComparison, int> countVariations = comparaison =>
{
   return comparaison.ContentA.Select((c, index) => c != 
   
   (index>comparaison.ContentB.Length-1?'_':comparaison.ContentB[index])//a plus long
   
   ).Where(c => c==true).Count()
   +(comparaison.ContentB.Length>comparaison.ContentA.Length?comparaison.LengthVariation:0/*b plus long*/);
};

 var lines = diffLines.Select((diffLine, index) => (number: countVariations(diffLine), line: index));

// TODO: 10 Afficher pour chaque ligne différente, le nombre de variations
foreach(var line in lines)
{
    Console.WriteLine("Ligne" + line.line + " : " + line.number + " differences");
}
